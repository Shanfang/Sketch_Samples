...
double Sketch::Median(double *x, int n) {
  ...
  // bubble sort, this can be optimized to O(NlogN)
  bool rpt = true;
  while (rpt) {
    rpt = false;
...
double AGMS_Sketch::Size_Of_Join(Sketch *s1) {
 ...
 // since AMS-sketches are randomized schemes, the average-case analysis
 // is a more appropriate characterization than a worst-case analysis 
